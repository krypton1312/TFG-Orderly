...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT new com.yebur.backendorderly.product.ProductResponse(p.id, p.name, p.price, p.stock, p.category.id, p.destination) FROM Product p")
    List<ProductResponse> findAllProductDTO();

    @Query("SELECT new com.yebur.backendorderly.product.ProductResponse(p.id, p.name, p.price, p.stock, p.category.id, p.destination) FROM Product p WHERE p.category.id = :categoryId")
