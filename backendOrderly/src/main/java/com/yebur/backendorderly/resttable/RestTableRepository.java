...
public interface RestTableRepository extends JpaRepository<RestTable, Long>{
    ...
    @Query("SELECT new com.yebur.backendorderly.resttable.RestTableResponse(rt.id, rt.number, rt.status, rt.position) FROM RestTable rt")
    List<RestTableResponse> findAllRestTableDTO();

    @Override
    Optional<RestTable> findById(Long id);

    @Query("SELECT new com.yebur.backendorderly.resttable.RestTableResponse(rt.id, rt.number, rt.status, rt.position) FROM RestTable rt WHERE rt.id= :id")
